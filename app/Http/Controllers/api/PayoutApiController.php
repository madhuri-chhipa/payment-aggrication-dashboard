...
use App\Services\BluswapPayoutService;
...
class PayoutApiController extends Controller
{
  ...
  protected $bluswapService;

  public function __construct(BluswapPayoutService $bluswapService, UnionbankPayoutService $unionbankService, FinkedaPayoutService $finkedaService, BulkpePayoutService $bulkpeService, PaywizePayoutService $paywizeService, BuckboxPayoutService $buckboxService, RunpaisaPayoutService $runpaisaService)
  {
    ...
    $this->bluswapService = $bluswapService;
  }
  ...
  public function bluswapcallback(Request $request)
  {
    return $this->bluswapService->handlePayoutCallback();
  }
  public function payoutMoneyTransfer(Request $request)
  {
    $user = $this->authenticateRequest($request);
    if ($user instanceof \Illuminate\Http\JsonResponse) return $user;
    if (!$user) return response()->json(['status' => 'FAILED', 'message' => 'User Not Found , Please contact admin.'], 400);
    ...
    $payment_mode      = 'IMPS';
    $active_api        = $userApis->active_payout_api;
    //$active_api        = 'Buckbox';
    if ($userApis->wallet_type != 'payout_wallet') {
      return response()->json([
        'status'  => 'FAILED',
    ...
    if ($userApis->virtual_type == 'percentage') {
      if ($amount <= 500) {
        $charge_amount = round($userApis->pflat_charges, 2);
      } elseif ($amount > 500 && $amount <= 1000) {
        $charge_amount = round($userApis->pflat_charges_2, 2);
      } elseif ($amount > 1000) {
        $charge_amount = round(($amount * $userApis->payout_charges) / 100, 2);
      } else {
    ...
    if ($Payouttxn && $wallettxn) {
      ...
      } elseif ($active_api === 'Bluswap') {
        $api_response = $this->bluswapService->initiateTransaction($refid, $bene_account, $ifsc_code, $amount, $beneficiary_name, $email, $mobile);
      } else {
