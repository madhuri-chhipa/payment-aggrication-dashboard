...
use App\Services\FinkedaPayoutService;
use App\Services\UnionbankPayoutService;
use App\Models\UserWalletTransaction;
use Illuminate\Support\Facades\Log;

class PayoutServiceController extends Controller
{
  ...
  public function __construct(UnionbankPayoutService $unionbankService, FinkedaPayoutService $finkedaService, BulkpePayoutService $bulkpeService, PaywizePayoutService $paywizeService, BuckboxPayoutService $buckboxService, RunpaisaPayoutService $runpaisaService)
  {
  ...
  {
    return view('user.services.payout');
  }
  public function payoutsubmit(Request $request)
  {
    $request->validate([
      'reqLat'              => 'required|string|max:100',
      'reqLong'              => 'required|string|max:100',
      'bene_name'            => 'required|string|max:100',
      'bene_account_number'  => 'required|digits_between:9,18',
      'ifsc_code'            => 'required|string|size:11',
      'bank_name'            => 'nullable|string|max:100',
      'bank_branch'          => 'nullable|string|max:100',
      'bene_mobile'          => 'nullable|digits:10',
      'bene_email'           => 'nullable|email|max:100',
      'amount'               => 'required|numeric|min:1',
      'transfer_mode'        => 'required|in:IMPS,NEFT,RTGS',
    ]);
    ...
    if (
      !is_numeric($amount) || $amount < $userServices->minimum_transaction || $amount > $userServices->maximum_transaction
    ) {
      return response()->json([
        'status'  => 'FAILED',
        'message' => 'Amount must be between ' .
          $userServices->minimum_transaction .
          ' and ' .
          $userServices->maximum_transaction . '.'
      ], 400);
    }

    $bene_account     = $request->bene_account_number;
    $ifsc_code        = $request->ifsc_code;
    $beneficiary_name = $request->bene_name;
    $mobile           = $request->bene_mobile ?? $user->phone_number;
    $email            = $request->bene_email ?? $user->email;
    $payment_mode     = 'IMPS';
    $active_api       = $userServices->active_payout_api;
    $reqLat     = $request->reqLat ?? '26.273987709800267';
    $reqLong     = $request->reqLong ?? '73.00436536810876';
    //$active_api        = 'Buckbox';

    $charge_amount = 0;
    if ($userServices->wallet_type != 'payout_wallet') {
      return response()->json([
        'status'  => 'FAILED',
        'message' => 'Wallet is disabled. Please contact admin.'
      ], 400);
    }
    $charge_amount = 0;
    if ($userServices->virtual_type == 'percentage') {
      if ($amount <= 500) {
        $charge_amount = round((float) $userServices->pflat_charges, 2);
      } elseif ($amount > 500 && $amount <= 1000) {
        $charge_amount = round((float) $userServices->pflat_charges_2, 2);
      } elseif ($amount > 1000) {
        $charge_amount = round(((float) $amount * (float) $userServices->payout_charges) / 100, 2);
      } else {
        $charge_amount = round(((float) $amount * 1) / 100, 2);
      }
    } elseif ($userServices->virtual_type == 'flatrate') {
      if ($amount <= 1000) {
        $charge_amount = round((float) $userServices->pslab_1000, 2);
      } elseif ($amount <= 25000) {
        $charge_amount = round((float) $userServices->pslab_25000, 2);
      } elseif ($amount > 25000) {
        $charge_amount = round((float) $userServices->pslab_200000, 2);
      } else {
        $charge_amount = round(((float) $amount * 1) / 100, 2);
      }
    } else {
    ...
    $total_service_amount = $charge_amount + $gst_amount;
    $total_amount = round($amount + $total_service_amount, 2);
    $usable_balance = bcsub($user->payout_balance, $user->freeze_balance, 2);
    $checkamount         = number_format($total_amount, 2, '.', '');
    if (bccomp($usable_balance, $checkamount, 2) === -1) {
      return response()->json([
        'status'  => 'FAILED',
        'message' => 'Insufficient balance. Please add balance in your wallet.'
      ], 400);
    }
    try {
    ...
    if ($Payouttxn && $wallettxn) {
      if ($active_api === 'Flipzik') {
      ...
      } elseif ($active_api === 'Finkeda') {
        $api_response = $this->finkedaService->initiateTransaction(
          $refid,
          $bene_account,
          $ifsc_code,
          $amount,
        ...
        $api_response = $this->unionbankService->initiateTransaction($refid, $bene_account, $ifsc_code, $amount, $beneficiary_name, $email, $mobile);
      } else {
  ...
  private function generateFinkedaUrn(): string
  {
    do {
      ...
      $exists = PayoutTransaction::where('txn_id', $urn)->exists();
    } while ($exists);

    return $urn;
  }

  public function payoutCheckstatus(Request $request)
  {
  ...
  public function payoutcheckbalance(Request $request)
  {
    $user = $this->authenticateRequest($request);
    if ($user instanceof \Illuminate\Http\JsonResponse) return $user;
    if (!$user) {
    ...
    return response()->json([
      ...
      'wallet' => [
        'payout_balance' => $user->payout_balance ?? 0.00,
        'payin_balance' => $user->payin_balance ?? 0.00,
      ]
    ]);
  }
  ...
  private function debitUserWalletAmount($user, $refid, $amount, $charge_amount, $gst_amount)
  {
    ...
    try {
      ...
      DB::table('user_wallet_transactions')->insert([
        ...
        'credit'          => 0.00,
        'debit'           => $total_amount,
        'description'     => $user->uid . '/PAYOUT/' . $refid,
        'created_at'      => now(),
        'updated_at'      => now(),
      ]);
