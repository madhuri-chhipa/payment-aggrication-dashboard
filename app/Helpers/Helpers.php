...
class Helpers
{
   ...
   */
  public static function generateUniqueUid($table = 'users', $column = 'uid')
  {
    do {
      ...
      $exists = DB::table($table)->where($column, $uid)->exists();
    } while ($exists);

    return $uid;
  }

  public static function storeEncryptedDoc($file, $userId, $docNumber)
  {
    $safeDocNumber = preg_replace('/[^A-Za-z0-9_-]/', '', $docNumber);
    $extension = $file->getClientOriginalExtension();

    $folder = "docs/{$userId}";
    Storage::disk('public')->makeDirectory($folder);

    $fileName = "{$safeDocNumber}.{$extension}.enc";

    $base64 = base64_encode(file_get_contents($file->getRealPath()));
    $encrypted = Crypt::encryptString($base64);

    Storage::disk('public')->put("{$folder}/{$fileName}", $encrypted);

    return "public/{$folder}/{$fileName}";
  }

  /**
   * Decrypt encrypted document and return raw file
   */
  public static function decryptDocument($path)
  {
    if (!$path || !Storage::disk('public')->exists(str_replace('public/', '', $path))) {
      return null;
    }

    $encrypted = Storage::disk('public')->get(str_replace('public/', '', $path));
    $base64 = Crypt::decryptString($encrypted);
    $raw = base64_decode($base64);

    // Get mime type from the original file extension
