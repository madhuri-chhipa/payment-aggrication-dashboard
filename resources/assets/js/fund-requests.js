$(document).ready(function () {
  const table = $('#fund-request-table').DataTable({
    ...
    columns: [
      ...
      { data: 'transaction_utr', width: '200px', className: 'text-end' },
  ...
  $('#resetFilter').click(function () {
    $('#filter-reqid, #filter-user, #filter-wallet-txn-id, #filter-sender-account-number').val('');
  ...
  $(document).on('click', '.view-request', function () {
    ...
    // AJAX call
    $.ajax({
      url: '/admin/fund-request/details/' + id,
      type: 'GET',
  ...
  $(document).on('click', '.change-status', function () {
    let id = $(this).data('id');
    let status = $(this).data('status'); // A = Approve, R = Reject
    let actionText = status === 'A' ? 'approve' : 'reject';
    let isReject = status === 'R';
  
    Swal.fire({
      title: 'Are you sure?',
      text: 'You are about to ' + actionText + ' this fund request.',
      icon: 'warning',
      input: 'textarea',
      inputLabel: 'Remark',
      ...
      if (result.isConfirmed) {
        updateFundStatus(id, status, result.value); // send remark
      }
    });
  });
  
  function updateFundStatus(id, status, remark) {
    $.ajax({
      url: window.routes.updateFundStatusUrl,
      type: 'POST',
      data: {
      ...
      success: function (res) {
        ...
        // Reload DataTable
        $('#fund-request-table').DataTable().ajax.reload(null, false);
  
        // If sidebar is open, refresh it
        if ($('#fundRequestSidebar').hasClass('show')) {
          $('.view-request[data-id="' + id + '"]').click();
        }
      },
      error: function () {
        $('#fund-request-table').DataTable().ajax.reload(null, false);
