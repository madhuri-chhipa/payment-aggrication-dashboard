...
@section('page-script')
  <script>
    window.routes = {
      txnData: "{{ route('admin.bbps-transactions.data') }}",
      viewTxn: "{{ url('/admin/bbps/transaction') }}"
    };
  </script>
  @vite('resources/assets/js/bbps-transactions.js')
  <script src="https://code.jquery.com/jquery-3.7.1.min.js"></script>
  <script>
    $(document).on('click', '.check-status', function() {
      let id = $(this).data('id');
      $.ajax({
        url: `/admin/bbps/txn-status/${id}`,
        type: "GET",
        success: function(res) {
          ...
          if (res.status) {
            ...
            toastr.success(
              "Transaction Status: " + res.txn_status +
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-4 g-3 align-items-end">
        ...
        <div class="col">
          <label>User</label>
          <input type="text" id="filter-user" placeholder="Name / Email / Phone" class="form-control">
        </div>
